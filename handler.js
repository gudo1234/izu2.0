...
export async function handler(chatUpdate) {
    ...
    for (let message of chatUpdate.messages) {
        ...
        try {
            ...
            try {
                // Inicio del handler completo dentro del bucle
                let user = global.db.data.users[m.sender]
                if (typeof user !== 'object') global.db.data.users[m.sender] = {}
                if (user) {
                    if (!isNumber(user.exp)) user.exp = 0
                    if (!isNumber(user.coin)) user.coin = 10
                    // ... resto de validaciones de usuario
                } else {
                    global.db.data.users[m.sender] = {
                        exp: 0,
                        coin: 10,
                        // ... resto de propiedades default
                    }
                }
                ...
                if (isSubbs && !isAllowed) continue

                if (opts['nyimak']) continue
                if (!m.fromMe && opts['self']) continue
                if (opts['swonly'] && m.chat !== 'status@broadcast') continue
                ...
                for (let name in global.plugins) {
                    ...
                    if ((usedPrefix = (match[0] || '')[0])) {
                        ...
                        if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
                            ...
                            if (!isROwner) {
                                if (!['grupo-unbanchat.js'].includes(name) && chat && chat.isBanned) continue
                                if (name != 'grupo-unbanchat.js' && name != 'owner-exec.js' && name != 'owner-exec2.js' && name != 'grupo-delete.js' && chat?.isBanned) continue
                                if (user.antispam > 2) continue
                                if (m.text && user.banned) {
                            ...
                            if (user.antispam2 && isROwner) continue
