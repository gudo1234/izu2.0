...
const handler = async (m, { conn, text, usedPrefix, command, args }) => {
  const e = '⚠️'; // Asumo que tienes la variable para iconos, si no la defines aquí
  ...
  try {
    ...
    // Si no es comando doc y dura más de 20 min, enviar como documento
    if (!sendAsDocument && durationMinutes > 20) sendAsDocument = true;
    ...
    // Aquí está el cambio: mostramos mensaje si se envía como documento por duración
    const caption = `
╭───── • ─────╮
  𖤐 \`YOUTUBE EXTRACTOR\` 𖤐
╰───── • ─────╯
