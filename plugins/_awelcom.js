...
export async function before(m, { conn, participants, groupMetadata }) {
  ...
  // 🔹 Fetch país y bandera
  let codigoPais = ''
  let bandera = ''
  try {
    let tel = user?.phone || who.replace('@s.whatsapp.net', '')
    let res = await fetch(`https://api.dorratz.com/v2/pais/${tel}`)
    let data = await res.json()
    codigoPais = data.código || ''
    bandera = data.bandera || ''
  } catch (e) {
    console.log('Error obteniendo país y bandera:', e)
  }
  ...
  if (chat.welcome && [27, 28, 32].includes(m.messageStubType)) {
    ...
    const titleWithFlag = `${accion} ${codigoPais} ${bandera}`
