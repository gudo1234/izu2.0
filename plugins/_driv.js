...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (carpetaID) {
    ...
    try {
      ...
      await explorarCarpeta(m, conn, carpetaID, visitadas)
      m.react('✅')
    } catch (err) {
      console.error(err)
...
async function explorarCarpeta(m, conn, carpetaID, visitadas) {
  if (visitadas.has(carpetaID)) return
  visitadas.add(carpetaID)
  ...
  for (const subId of idsCarpetas) {
    ...
    await esperar(1500)
    await explorarCarpeta(m, conn, subId, visitadas)
  }
}

// 🔧 DESCARGA DE ARCHIVOS GOOGLE DRIVE
