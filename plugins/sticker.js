...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  // Corregimos si no se detecta mime
  if (!mime && q?.fileLength && q?.mtype) {
    mime = q.mtype === 'videoMessage' ? 'video/mp4' :
           q.mtype === 'imageMessage' ? 'image/jpeg' :
           q.mtype === 'stickerMessage' ? 'image/webp' : '';
  }
  ...
  if (/webp|image|video|gif/.test(mime)) {
    ...
    try {
      img = await (await conn.downloadMediaMessage(q)).buffer;
    } catch (err) {
...
◈ Usa \`${usedPrefix + command} -a\` respondiendo a una imagen o video.`, m);
  ...
  try {
    if (selectedShape && /image|webp|url|video|gif/.test(mime)) {
      ...
      if (/video|gif/.test(mime)) {
        const format = /gif/.test(mime) ? 'gif' : 'mp4';
        const ffmpeg = spawn('ffmpeg', ['-f', format, '-i', 'pipe:0', '-vframes', '1', '-f', 'image2', '-']);
        ffmpeg.stdin.write(img);
        ffmpeg.stdin.end();
