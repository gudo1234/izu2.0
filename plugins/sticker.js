...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if (/webp|image|video/g.test(mime)) {
    ...
    return conn.reply(m.chat, `Envía o responde a una imagen, GIF o video corto con una de estas formas: ${Object.keys(shapeFlags).join(', ')}`, m);
  }
  ...
  try {
    ...
    if (isAnimated) {
      ...
      const tempOutput = path.join(tmpdir(), `output_${Date.now()}.webp`);
      fs.writeFileSync(tempInput, img);
      await convertToAnimatedSticker(tempInput, tempOutput);
      const animatedSticker = fs.readFileSync(tempOutput);
      fs.unlinkSync(tempInput);
      fs.unlinkSync(tempOutput);
      return await conn.sendMessage(m.chat, { sticker: animatedSticker }, { quoted: m });
...
function getSVGMask(shape, size) {
  ...
  switch (shape) {
    ...
    case 'heart': return `<svg width="${size}" height="${size}" viewBox="0 0 32 29.6"><path d="M23.6,0c-2.7,0-5.1,1.3-6.6,3.3C15.5,1.3,13.1,0,10.4,0C4.7,0,0,4.7,0,10.4c0,6,6.2,10.9,15.7,18.5L16,29.6l0.3-0.3 C25.8,21.3,32,16.4,32,10.4C32,4.7,27.3,0,21.6,0H23.6z" fill="black"/></svg>`;
